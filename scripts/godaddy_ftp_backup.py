...
def list_entries(ftp, remote_dir):
    try:
        return [(name, facts.get("type", "")) for name, facts in ftp.mlsd(remote_dir) if name not in {".", ".."}]
    except Exception:
        result = []
        for raw in ftp.nlst(remote_dir):
...
def download_tree(ftp, remote_dir, local_dir, top_level=True):
    ...
    for name, kind in list_entries(ftp, remote_dir):
        ...
        if kind == "dir":
            download_tree(ftp, remote_path, local_path, False)
        else:
...
def delete_remote_tree(ftp, remote_dir, top_level=True):
    for name, kind in list_entries(ftp, remote_dir):
        ...
        path = posixpath.join(remote_dir, name)
        if kind == "dir":
            delete_remote_tree(ftp, path, False)
            try:
                ftp.rmd(path)
            except Exception:
                pass
...
def verify_backup():
    ...
    if not (LOCAL_ROOT / "api").is_dir() or not (LOCAL_ROOT / "lib").is_dir():
