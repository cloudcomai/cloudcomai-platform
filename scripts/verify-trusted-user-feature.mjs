...
assert.match(attachmentApi, /is_trusted_user\(\(int\)\$user\['id'\], \(int\)\$a\['sender_id'\]\)/);
assert.match(requestApi, /is_trusted_user\(\(int\)\$user\['id'\], \(int\)\$a\['sender_id'\]\)/);
...
assert.match(profileUi, /Allow attachments from this user without requesting approval/);
