...
After this workflow is merged, open Actions → Build Android without EAS →
Run workflow → preview. Download the APK from the completed run's artifact.
Pull requests affecting mobile code also compile a preview APK for validation.
...
backend's HTTPS API URL. Without it, PR previews use example.invalid to allow
