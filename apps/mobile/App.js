  ...
  const pickImage = async useCamera => {
    ...
    try {
      const picked = await withAppLockExternalActivity(async () => {
        ...
        if (!permission.granted) throw new Error(useCamera ? 'Camera permission is required to take a photo.' : 'Photo library permission is required to choose a photo.');
        const options = {
          mediaTypes: ['images'],
          quality: 0.82,
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            return <Pressable onPress={() => setSelectedMessage(current => Number(current?.id) === Number(item.id) ? null : item)} onLongPress={() => setSelectedMessage(item)} style={[styles.messageBubble, mine && styles.myMessage, { backgroundColor: mine ? theme.colors.outgoing : theme.colors.incoming, borderColor: theme.colors.border }, selected && styles.selectedMessage, highlightedMessageId === Number(item.id) && styles.highlightedMessage]}>
              ...
              <MediaMessage message={item} autoDownload={privacy.media_auto_download} colors={messageColors} textScale={Number(themeSettings?.textScale || 1)} isVisible={visibleMessageIds.has(Number(item.id)) && !groupManagementOpen && !profileOpen} showActions={selected} />
              ...
              {selected ? <View style={styles.messageActions}>
                <Pressable accessibilityRole="button" accessibilityLabel={item.saved ? 'Unsave message' : 'Save message'} hitSlop={8} onPress={() => toggleSaved(item)} style={styles.messageActionButton}><Text style={[styles.messageActionIcon, { color: messageColors.text }]}>{item.saved ? '★' : '☆'}</Text></Pressable>
                <Pressable accessibilityRole="button" accessibilityLabel="Reply to message" hitSlop={8} onPress={() => { setReplyTo(item); if (editing) setComposer(messaging?.snapshot().drafts[String(chat.id)] || ''); setEditing(null); setSelectedMessage(null); }} style={styles.messageActionButton}><Text style={[styles.messageActionIcon, { color: messageColors.text }]}>↩</Text></Pressable>
                {mine && item.type === 'text' && Number(item.edit_count || 0) === 0 ? <Pressable accessibilityRole="button" accessibilityLabel="Edit message" hitSlop={8} onPress={() => { setEditing(item); setReplyTo(null); setComposer(item.body || item.text || ''); setSelectedMessage(null); }} style={styles.messageActionButton}><Text style={[styles.messageActionIcon, { color: messageColors.text }]}>✎</Text></Pressable> : null}
                <Pressable accessibilityRole="button" accessibilityLabel="Delete message" hitSlop={8} onPress={() => { setSelectedMessage(null); deleteMessage(item); }} style={styles.messageActionButton}><Text style={[styles.messageActionIcon, styles.messageDeleteAction]}>⌫</Text></Pressable>
              </View> : null}
            </Pressable>;
          }}
        />
...
function AppContent() {
  ...
  if (accountTool) return <AccountTools themeSettings={chatThemeSettings} key={accountTool} mode={accountTool} onBack={() => setAccountTool(null)} onLogout={logout} onSessionRotated={acceptRotatedSession} onOpenChat={id => { setInitialChatId(id); setAccountTool(null); setShowNotificationSettings(false); }} />;
...
const styles = StyleSheet.create({
  ...
  searchRow: { flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 18, paddingVertical: 10 }, contextBar: { minHeight: 52, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, borderTopWidth: 1, borderTopColor: '#dfe4ee', backgroundColor: '#f8faff' }, contextBarText: { flex: 1, minWidth: 0 }, contextBarLabel: { color: '#3157d5', fontWeight: '800', fontSize: 11 }, contextBarValue: { color: '#475569', fontSize: 12, marginTop: 2 }, contextBarClose: { color: '#64748b', fontSize: 24, paddingHorizontal: 8 }, replyPreview: { padding: 8, marginBottom: 8, borderLeftWidth: 3, borderLeftColor: '#3157d5', borderRadius: 6, backgroundColor: '#f8faff' }, replySender: { color: '#3157d5', fontSize: 10, fontWeight: '800' }, replyText: { color: '#64748b', fontSize: 11, marginTop: 2 }, messageActions: { flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center', gap: 4, marginTop: 5 }, messageActionButton: { width: 30, height: 30, borderRadius: 15, alignItems: 'center', justifyContent: 'center' }, messageActionIcon: { color: '#3157d5', fontSize: 16, fontWeight: '700', lineHeight: 20 }, messageActionText: { color: '#3157d5', fontSize: 11, fontWeight: '700' }, selectedMessage: { borderWidth: 1.5, borderColor: '#3157d5' }, highlightedMessage: { borderWidth: 2, borderColor: '#f59e0b' }, attachmentCard: { width: '94%', maxHeight: '82%', padding: 18, borderRadius: 18, backgroundColor: '#fff' }, attachmentMenuOverlay: { flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 22, backgroundColor: '#0008' }, attachmentMenuCard: { width: '100%', maxWidth: 420, paddingHorizontal: 20, paddingTop: 18, paddingBottom: 14, borderRadius: 3, backgroundColor: '#fff', elevation: 8, shadowColor: '#000', shadowOpacity: 0.2, shadowRadius: 12 }, attachmentMenuHeader: { flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }, attachmentMenuClose: { width: 30, height: 30, alignItems: 'center', justifyContent: 'center' }, attachmentMenuCloseText: { color: '#374151', fontSize: 25, lineHeight: 27 }, attachmentMenuHint: { marginTop: -4, marginBottom: 22, color: '#172033', fontSize: 16, lineHeight: 20 }, attachmentMenuActions: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 8 }, attachmentMenuOption: { flex: 1, minHeight: 46, alignItems: 'center', justifyContent: 'center' }, attachmentMenuOptionText: { color: '#008577', fontSize: 13, fontWeight: '700', textAlign: 'center' }, attachmentMenuCancel: { minHeight: 44, justifyContent: 'center', paddingHorizontal: 4 }, attachmentTitle: { fontSize: 18, fontWeight: '800', color: '#172033', marginBottom: 12 }, attachmentPreview: { width: '100%', height: 300, borderRadius: 12, backgroundColor: '#f3f4f6' }, documentPreview: { minHeight: 180, alignItems: 'center', justifyContent: 'center', padding: 20, borderRadius: 12, backgroundColor: '#f8faff' }, documentIcon: { fontSize: 50 }, documentName: { marginTop: 10, fontSize: 15, fontWeight: '800', color: '#172033', textAlign: 'center' }, documentSize: { marginTop: 5, color: '#68748a', fontSize: 11 }, attachmentSending: { marginTop: 10, flexDirection: 'row', alignItems: 'center', gap: 8 }, attachmentProgress: { color: '#3157d5', fontWeight: '700' }, attachmentError: { marginTop: 10, padding: 10, borderRadius: 10, backgroundColor: '#fff1f2', flexDirection: 'row', justifyContent: 'space-between' }, attachmentErrorText: { flex: 1, color: '#b91c1c' }, attachmentActions: { marginTop: 14, flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center', gap: 18 }, attachmentSend: { paddingVertical: 11, paddingHorizontal: 18, borderRadius: 10, backgroundColor: '#3157d5' }, attachmentSendText: { color: '#fff', fontWeight: '800' }, cancelText: { color: '#68748a', fontWeight: '700' }, messageDeleteAction: { color: '#b91c1c' }, moderationMessage: { alignSelf: 'center', maxWidth: '90%', marginVertical: 8, padding: 12, borderRadius: 12, backgroundColor: '#fff7ed', borderWidth: 1, borderColor: '#fed7aa' }, moderationTitle: { color: '#9a3412', fontSize: 11, fontWeight: '800', textAlign: 'center' }, moderationText: { marginTop: 4, color: '#7c2d12', fontSize: 12, lineHeight: 17, textAlign: 'center' }, moderationTime: { marginTop: 5, color: '#a16207', fontSize: 9, textAlign: 'center' }, searchLink: { color: '#3157d5', fontWeight: '600' }, searchBox: { paddingHorizontal: 14, paddingBottom: 8 }, messageDelete: { alignSelf: 'flex-end', color: '#68748a', fontSize: 11, paddingTop: 8 }, sender: { color: '#68748a', fontSize: 11, fontWeight: '700', marginBottom: 5 },
