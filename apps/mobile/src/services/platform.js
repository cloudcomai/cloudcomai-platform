...
export const uploadAttachmentAsset=(asset,parameters={})=>{const {onProgress,extraFiles,multipartPartMode='expo-file',...formParameters}=parameters;return uploadMobileFile(ApiRoute.UPLOAD_ATTACHMENT,asset,{fieldName:'file',fallbackName:'attachment',parameters:{...formParameters,original_filename:normalizeUploadAsset(asset,{fallbackName:'attachment'}).name},multipartPartMode,onProgress,extraFiles});};
...
export async function downloadAttachmentPreview(attachment,{download=false}={}){if(!attachment?.id)throw new ApiError('Attachment preview is unavailable.');const token=await sessionManager.getToken();if(!token)throw new ApiError('Authentication is required to access this attachment.',{status:401});const directory=new Directory(Paths.cache,download?'cloudcomai-attachment-downloads':'cloudcomai-attachment-previews');directory.create({intermediates:true,idempotent:true});const version=String(attachment.updated_at||attachment.created_at||attachment.file_size||'1').replace(/[^a-z0-9._-]/gi,'_');const extension=attachmentPreviewExtension(attachment);const file=new File(directory,`${Number(attachment.id)}-${version}.${extension}`);if(file.exists&&Number(file.size||0)>0)return file;try{return await File.downloadFileAsync(buildApiUrl(API_BASE_URL,ApiRoute.ATTACHMENT,{id:attachment.id,...(!download?{preview:1}:{})}),file,{headers:{Authorization:`Bearer ${token}`}});}catch(error){if(file.exists){try{file.delete();}catch{}}throw error;}}
