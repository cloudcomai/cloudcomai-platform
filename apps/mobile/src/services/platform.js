...
export async function createMobileMultipartBody(asset,{fieldName='file',parameters={}}={},FormDataCtor=globalThis.FormData){if(typeof FormDataCtor!=='function')throw new ApiError('Multipart upload is unavailable on this device.');const normalized=normalizeUploadAsset(asset,{fallbackName:'attachment'});const form=new FormDataCtor();const response=await fetch(normalized.uri);if(!response.ok)throw new ApiError(`Unable to read the selected file (status ${response.status}).`);const blob=await response.blob();const typedBlob=blob.type===normalized.mimeType?blob:blob.slice(0,blob.size,normalized.mimeType);form.append(fieldName,typedBlob,normalized.name);for(const[key,value]of Object.entries(parameters)){if(value!==undefined&&value!==null)form.append(key,String(value));}return form;}
export async function uploadMobileFile(route,asset,{fieldName='file',parameters={},fallbackName='upload',fallbackMime='application/octet-stream',maxBytes=25*1024*1024}={}){const normalized=normalizeUploadAsset(asset,{fallbackName,fallbackMime});const file=new File(normalized.uri);if(!file.exists)throw new ApiError('The selected file is no longer available.');const size=Number(file.size??normalized.size??0);if(!Number.isFinite(size)||size<=0)throw new ApiError('The selected file is empty.');if(size>maxBytes)throw new ApiError(`The selected file must be ${Math.floor(maxBytes/1024/1024)} MB or smaller.`);const token=await sessionManager.getToken();const formData=await createMobileMultipartBody(normalized,{fieldName,parameters:{...parameters,original_filename:parameters.original_filename||normalized.name}});const response=await fetch(buildApiUrl(API_BASE_URL,route),{method:'POST',headers:token?{Authorization:`Bearer ${token}`}:{},body:formData});const body=await response.text();return parseUploadResult({status:response.status,body,headers:Object.fromEntries(response.headers.entries())},token);}
export const uploadAttachmentAsset=(asset,parameters={})=>uploadMobileFile(ApiRoute.UPLOAD_ATTACHMENT,asset,{fieldName:'file',fallbackName:'attachment',parameters:{...parameters,original_filename:normalizeUploadAsset(asset,{fallbackName:'attachment'}).name}});
