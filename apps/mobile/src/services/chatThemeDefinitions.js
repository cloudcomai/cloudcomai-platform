...
export function readableMessageColor(background, preferred = LIGHT_THEME_TEXT) {
  const luminance = hex => {
    const channels = hex.slice(1).match(/.{2}/g).map(pair => parseInt(pair, 16) / 255)
      .map(value => value <= 0.04045 ? value / 12.92 : ((value + 0.055) / 1.055) ** 2.4);
    return channels[0] * 0.2126 + channels[1] * 0.7152 + channels[2] * 0.0722;
  };
  if (!/^#[0-9a-f]{6}$/i.test(background) || !/^#[0-9a-f]{6}$/i.test(preferred)) return preferred;
  const surface = luminance(background);
  const text = luminance(preferred);
  if ((Math.max(surface, text) + 0.05) / (Math.min(surface, text) + 0.05) >= 4.5) return preferred;
  return (surface + 0.05) / 0.05 >= 1.05 / (surface + 0.05) ? '#000000' : '#ffffff';
}
