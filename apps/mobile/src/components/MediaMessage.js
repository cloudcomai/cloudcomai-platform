...
function AttachmentApproval({ attachment, message, showActions = false, onForward }) {
    ...
    try {
      ...
      if (nextStatus === 'APPROVED') {
        if (requestType === 'DOWNLOAD') {
          await downloadAttachmentPreview(attachment, { download: true });
        } else if (requestType === 'FORWARD') {
          onForward?.();
        }
        return;
      }
