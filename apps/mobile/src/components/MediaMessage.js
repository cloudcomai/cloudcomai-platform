...
export default function MediaMessage({ message, autoDownload }) {
  ...
  if (!attachment) return <Pressable onLongPress={() => setForwardOpen(true)}><View>
    {message.type === 'forwarded_text' ? <Text style={styles.forwardedLabel}>Forwarded</Text> : null}
    <Text style={styles.text}>{message.body || ''}</Text>
    {message.sender_id ? <Pressable style={styles.profileLink} onPress={() => setProfileOpen(true)}><Text style={styles.profileLinkText}>View {message.sender_name || 'sender'} profile</Text></Pressable> : null}
    <Pressable style={styles.forwardLink} onPress={() => setForwardOpen(true)}><Text style={styles.link}>↗ Forward</Text></Pressable>
