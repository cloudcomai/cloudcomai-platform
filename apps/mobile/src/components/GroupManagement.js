...
export default function GroupManagement({ visible, group, user, onClose, onGroupUpdated, onGroupDeleted }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setName(group.name || '');
      setImageVersion(group.image_version || Date.now());
    } catch (e) {
      setError(e.message || 'Unable to load group members.');
    } finally {
      setBusy(false);
    }
  }, [visible, group?.id, group?.name, group?.image_version]);
  ...
  const imageSource = mediaUrl('group', group.id, imageVersion);
  return (
        ...
        <ScrollView contentContainerStyle={styles.content}>
          <View style={styles.card}>
            ...
            <View style={styles.photoWrap}>
              <ProfileImagePreview source={imageSource} fallback={group.name?.[0]?.toUpperCase() || 'G'} label={`${group.name || 'group'} profile photo`} size={96} />
