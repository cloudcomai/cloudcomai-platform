    ...
    <View style={styles.container}>
      ...
      {open ? (
        <View style={styles.menu}>
          <FlatList
            ...
            renderItem={({ item }) => (
              <Pressable style={styles.roomRow} onPress={() => openRoom(item)} disabled={joining}>
      ...
      {!open ? (
        <FlatList
          ...
          renderItem={({ item }) => (
            <Pressable style={styles.selectedRoom} onPress={() => openRoom(item)} disabled={joining}>
              <Text style={styles.selectedRoomName}>{item.name}</Text>
              <Text style={styles.selectedRoomSub}>{joining ? 'Opening…' : 'Tap to open this public chat'}</Text>
            </Pressable>
          )}
          ListEmptyComponent={<Text style={styles.empty}>Choose a city or town above to join its public chat room.</Text>}
        />
      ) : null}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  dropdownLabel: { color: '#68748a', fontSize: 10, fontWeight: '700', textTransform: 'uppercase' },
  dropdownValue: { marginTop: 4, color: '#172033', fontSize: 15, fontWeight: '700' },
  chevron: { color: '#3157d5', fontSize: 24, paddingLeft: 10 },
  menu: { marginTop: 8, maxHeight: 430, borderWidth: 1, borderColor: '#e2e7f0', borderRadius: 12, backgroundColor: '#fff', overflow: 'hidden' },
  menuList: { flexGrow: 0 },
  ...
  cityIconText: { color: '#3157d5', fontWeight: '800' },
  roomMeta: { flex: 1, minWidth: 0, marginLeft: 10 },
  roomName: { color: '#172033', fontSize: 14, fontWeight: '800' },
  roomSub: { marginTop: 3, color: '#7a8497', fontSize: 11 },
  roomAction: { color: '#3157d5', fontWeight: '800', fontSize: 12 },
  selectedList: { flexGrow: 1, paddingTop: 14 },
  selectedRoom: { padding: 16, borderRadius: 12, backgroundColor: '#f8faff', borderWidth: 1, borderColor: '#e2e7f0' },
  selectedRoomName: { color: '#172033', fontSize: 16, fontWeight: '800' },
  selectedRoomSub: { marginTop: 5, color: '#68748a', fontSize: 12 },
