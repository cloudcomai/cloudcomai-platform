...
export default function PublicChatsList({ onOpenChat }) {
  ...
  const load = useCallback(async (refresh = false, silent = false, search = query) => {
    ...
    try {
      ...
      const nextFavorites = Array.isArray(data.favorites) ? data.favorites : nextRooms.filter(room => room.joined);
      setSelected(current => current ? [...nextRooms, ...nextFavorites].find(room => Number(room.id) === Number(current.id)) || null : null);
    } catch (e) {
  ...
  const openRoom = room => {
    ...
    setSelected(room); setOpen(false);
  ...
  if (loading && !rooms.length && !query) return <View style={styles.loadingWrap}><ActivityIndicator color="#3157d5" /><Text style={styles.loadingText}>Loading public chat rooms…</Text></View>;
