...
export default function PrivacySettings({ onBack }) {
  ...
  const load = useCallback(async () => {
    setError('');
    const [privacyResponse, backupResponse] = await Promise.allSettled([platformApi.getPrivacySettings(), platformApi.getAccountBackupStatus()]);
    if (privacyResponse.status === 'fulfilled') setData(privacyResponse.value.data);
    else setError(privacyResponse.reason?.message || 'Unable to load privacy settings.');
    if (backupResponse.status === 'fulfilled') setBackup(backupResponse.value.data.backup || {});
    else {
      setBackup({ configured: false, unavailable: true });
      setError(backupResponse.reason?.message || 'Cloud backup is unavailable.');
    }
  }, []);
  useEffect(() => { load(); }, [load]);
  const perform = async action => {
    ...
    try { await action(); }
    catch (actionError) { setProgress(''); setError(actionError.message || 'Unable to update settings.'); }
  ...
  const createBackup = () => perform(async () => {
    ...
    assertBackupConnection(backup, await Network.getNetworkStateAsync());
    setProgress('Creating encrypted backup');
  ...
  const restoreBackup = () => Alert.alert('Restore backup?', 'Restore your saved preferences, contacts, and available messages? Expired or deleted messages and rooms you have left cannot be restored.', [
  ...
  const requestDeletion = () => Alert.alert('Delete CloudComAI account?', 'This sends an account and associated-data deletion request. You may be asked to verify account control. This cannot remove copies independently saved by recipients.', [
    { text: 'Cancel', style: 'cancel' },
    { text: 'Request deletion', style: 'destructive', onPress: () => perform(async () => {
      const response = await platformApi.requestAccountDeletion('DELETE');
      setProgress(response.data?.message || 'Account deletion request received');
      Alert.alert('Request received', 'Your deletion request has been recorded. CloudComAI will process it subject to any specific lawful retention requirements.');
    }) },
  ]);
  ...
  return <SafeAreaView style={styles.page}>
    ...
    {!data ? <><ActivityIndicator style={{ margin: 30 }} /><Pressable onPress={load}><Text style={styles.link}>Retry</Text></Pressable></> : <ScrollView contentContainerStyle={styles.content}>
      ...
      <View style={styles.card}><Text style={styles.heading}>Delete account</Text><Text style={styles.note}>Request deletion of your CloudComAI account and associated personal data. This is different from signing out or uninstalling the app.</Text><Pressable disabled={busy} onPress={requestDeletion}><Text style={styles.dangerLink}>Request account deletion</Text></Pressable></View>
