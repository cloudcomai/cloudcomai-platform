...
import { assertBackupConnection } from '../src/utils/accountBackup.js';
...
test('account backup UI separates cloud backup from JSON export', () => {
  ...
  assert.match(ui, /Creating encrypted backup/);
  assert.match(ui, /Backup completed/);
});


test('manual backups enforce connection and Wi-Fi preferences', () => {
  assert.doesNotThrow(() => assertBackupConnection({ wifi_only: true }, { type: 'WIFI', isConnected: true, isInternetReachable: true }));
  assert.throws(() => assertBackupConnection({ wifi_only: true }, { type: 'CELLULAR', isConnected: true }), /Wi-Fi/);
  assert.throws(() => assertBackupConnection({}, { type: 'UNKNOWN', isConnected: true }), /Wi-Fi/);
  assert.doesNotThrow(() => assertBackupConnection({ wifi_only: false }, { type: 'CELLULAR', isConnected: true }));
  assert.throws(() => assertBackupConnection({ wifi_only: false }, { type: 'WIFI', isConnected: false }), /internet/);
  assert.throws(() => assertBackupConnection({ wifi_only: false }, { type: 'WIFI', isConnected: true, isInternetReachable: false }), /internet/);
});
