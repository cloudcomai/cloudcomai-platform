...
export default function NotificationPanel({ apiBridge, close, onUnreadChange }) {
  ...
  useEffect(() => { apiBridge('v1/notifications', { method: 'GET', query: { limit: 50 } }).then(data => { const count = Number(data.unread_count || 0); setNotifications(data.notifications || []); setUnread(count); onUnreadChange?.(count); }).catch(() => {}); }, [apiBridge, onUnreadChange]);
  const update = next => { const value = { ...preferences, ...next }; setPreferences(value); localStorage.setItem(KEY, JSON.stringify(value)); };
  const markAllRead = async () => { await apiBridge('v1/notifications/read', { method: 'POST', body: JSON.stringify({ all: true }) }); setNotifications(items => items.map(item => ({ ...item, read_at: item.read_at || new Date().toISOString() }))); setUnread(0); onUnreadChange?.(0); };
