...
`GET v1/users/profile?id=<user-id>` returns another user's display name, derived age,
gender, and optional email/mobile details when the authenticated viewer shares an
active private conversation with that user and neither user has blocked the other.
