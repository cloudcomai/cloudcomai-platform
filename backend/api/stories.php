...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  $st->execute([$user['id'],$type,$content,$aud]);
  out(['story_id'=>(int)db()->lastInsertId(),'expires_in_hours'=>36],201);
}
if($_SERVER['REQUEST_METHOD']==='GET'){
  $st=db()->prepare('SELECT s.id,s.user_id,s.type,s.content,s.audience,s.created_at,s.expires_at,u.name FROM stories s JOIN users u ON u.id=s.user_id WHERE s.expires_at>UTC_TIMESTAMP() AND s.deleted_at IS NULL ORDER BY s.created_at DESC LIMIT 100');
  $st->execute();
