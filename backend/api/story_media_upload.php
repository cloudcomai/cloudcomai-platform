...
if (!move_uploaded_file($file['tmp_name'], $destination)) fail('Unable to save Ring Bells media', 500);

$baseUrl = rtrim((string)($config['app']['base_url'] ?? ''), '/');
$mediaUrl = $baseUrl . '/story-media.php?user_id=' . (int)$user['id'] . '&file=' . rawurlencode($filename);
out(['type' => $kind, 'mime_type' => $mime, 'url' => $mediaUrl, 'filename' => $filename], 201);
