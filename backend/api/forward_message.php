...
$sourceStmt = db()->prepare('
    SELECT m.id,m.chat_id,m.sender_id,m.type,m.body,m.expires_at,c.type AS chat_type
    FROM messages m
    INNER JOIN chats c ON c.id=m.chat_id
    INNER JOIN chat_members source_member ON source_member.chat_id=m.chat_id AND source_member.user_id=? AND source_member.status="active"
...
try {
    ...
    foreach ($chatIds as $chatId) {
        $destination = $pdo->prepare('
            SELECT c.id,c.type,c.name,c.retention_seconds,cm.status,COALESCE(cus.cleared_through_message_id,0) AS cleared_through_message_id
            FROM chats c
            INNER JOIN chat_members cm ON cm.chat_id=c.id AND cm.user_id=? AND cm.status="active"
            LEFT JOIN chat_user_states cus ON cus.chat_id=c.id AND cus.user_id=?
            WHERE c.id=? AND c.type IN ("private","group")
            LIMIT 1
        ');
        $destination->execute([$user['id'], $user['id'], $chatId]);
        ...
        $insert = $pdo->prepare('
            INSERT INTO messages(chat_id,sender_id,type,body,reply_to_message_id,forwarded_from_message_id,expires_at,created_at)
            VALUES(?,?,"text",?,NULL,?,?,UTC_TIMESTAMP())
        ');
        $insert->execute([$chatId, $user['id'], $source['body'], $messageId, $expires]);
