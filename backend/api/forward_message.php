...
$sourceStmt = db()->prepare('SELECT m.id,m.chat_id,m.sender_id,m.type,m.body,m.expires_at,c.type AS chat_type,ma.id AS attachment_id,ma.original_filename,ma.stored_filename,ma.storage_path,ma.mime_type,ma.file_size,ma.download_policy FROM messages m INNER JOIN chats c ON c.id=m.chat_id INNER JOIN chat_members source_member ON source_member.chat_id=m.chat_id AND source_member.user_id=? AND source_member.status="active" LEFT JOIN message_user_states mus ON mus.message_id=m.id AND mus.user_id=? LEFT JOIN message_attachments ma ON ma.message_id=m.id WHERE m.id=? AND m.deleted_for_everyone=0 AND COALESCE(mus.hidden,0)=0 AND (m.expires_at IS NULL OR m.expires_at>UTC_TIMESTAMP()) LIMIT 1');
...
try {
    ...
    foreach ($chatIds as $chatId) {
        ...
        if ($hasAttachment) {
            $copyAttachment = $pdo->prepare('INSERT INTO message_attachments(message_id,original_filename,stored_filename,storage_path,mime_type,file_size,download_policy,created_at) VALUES(?,?,?,?,?,?,?,UTC_TIMESTAMP())');
            $forwardedStoredName = $newId . '-forward-' . basename((string)$source['stored_filename']);
            $sourcePath = (string)$source['storage_path'];
            $forwardPath = dirname($sourcePath) . DIRECTORY_SEPARATOR . $forwardedStoredName;
            if (!@copy($sourcePath, $forwardPath)) throw new RuntimeException('Unable to copy forwarded attachment');
            $copyAttachment->execute([$newId, $source['original_filename'], $forwardedStoredName, $forwardPath, $source['mime_type'], $source['file_size'], $source['download_policy']]);
        }
