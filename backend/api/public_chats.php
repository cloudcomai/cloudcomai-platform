...
if ($method === 'POST') {
    ...
    try {
        ...
        $pdo->prepare("INSERT INTO chat_members(chat_id,user_id,role,status,joined_at) VALUES(?,?,'member','active',UTC_TIMESTAMP()) ON DUPLICATE KEY UPDATE status='active',joined_at=UTC_TIMESTAMP()")->execute([$roomId,$user['id']]);
...
if ($method === 'DELETE') {
    ...
    try {
        ...
        $pdo->prepare("UPDATE chat_user_states SET hidden=1,notifications_muted=1,updated_at=UTC_TIMESTAMP() WHERE chat_id=? AND user_id=?")->execute([$roomId,$user['id']]);
        $pdo->commit();
